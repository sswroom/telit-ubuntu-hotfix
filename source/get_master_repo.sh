...
cd $TARGETPATH
VERSION=$(sed -n '/^VERSION = \([0-9]*\)$/s//\1/p' Makefile)
PATCHLEVEL=$(sed -n '/^PATCHLEVEL = \([0-9]*\)$/s//\1/p' Makefile)
SUBLEVEL=$(sed -n '/^SUBLEVEL = \([0-9]*\)$/s//\1/p' Makefile)
EXTRAVERSION=$(sed -n '/^EXTRAVERSION = \([-,a-z,0-9]*\)$/s//\1/p' Makefile)
cd ..
mv "$TARGETPATH" "$TARGETPATH-$VERSION.$PATCHLEVEL.$SUBLEVEL"
